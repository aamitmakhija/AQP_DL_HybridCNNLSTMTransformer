#!/usr/bin/env python3
# c_dataprep/03_feature_eng.py  (STRICT YAML, robust to missing pm25 aliases; no hardcoding)
...
def engineer():
    ...
    # Headers: pm25 aliases are OPTIONAL now (robust to missing key)
    pm25_aliases = (
        (((cfg.get("data", {}) or {}).get("headers", {}) or {})
         .get("airquality", {}) or {}).get("pm25")
        or [y_col]
    )
    ...
    io_format     = _req(out_cfg, ["format"], "features.output.format")
    ...
    # Split file names + format (strict)
    ...
    if train.empty:
        base_df = pd.read_parquet(ds_dir) if ds_dir.exists() else pd.DataFrame()
    else:
    ...
    # Enforce YAML primary ID name; drop stray generic 'id' if it co-exists
    if "id" in base_df.columns and id_col in base_df.columns and "id" != id_col:
        base_df = base_df.drop(columns=["id"])

    # Build features
    df = base_df

    # 1) target lags (if requested)
    if lags_hours:
        if y_col in df.columns:
            y_eff = y_col
        else:
            y_eff = _pick(df.columns, pm25_aliases)
        if y_eff:
            df = _create_target_lags(df, id_col=id_col, y_col=y_eff, lags=lags_hours)
        else:
            print(f"[warn] target '{y_col}' (or its aliases) not found; skipping lags")

    # 2) rolling stats for configured base numeric features (excluding target)
    ...
    # Manifest: numeric X only; exclude id/time/target
